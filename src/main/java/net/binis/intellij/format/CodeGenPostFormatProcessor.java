                        ...
                        if (firstElement instanceof PsiJavaToken token && ".".equals(token.getText()) && token.getParent() instanceof PsiReferenceExpression expression) {
                            ...
                            if (nonNull(call)) {
                                ...
                                if (nonNull(type)) {
                                    ...
                                    if ("save".equals(name) || ("done".equals(name) || "select".equals(name) || "where".equals(name) || "ensure".equals(name) || "saveAndFlush".equals(name) && !typeName.contains("Modify")) && params == 0) {
                                        var line = i;
                                        if (indents > 0) {
                                            indents--;
                                            var ind = indents;
